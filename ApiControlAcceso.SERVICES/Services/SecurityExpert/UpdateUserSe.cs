        ...
        public async Task<bool> Update(SeInfoDto model)
        {
            try
            {
                ...
                foreach (var item in model.InfoTarjet)
                {
                    ...
                    {
                        for (int i = 0; i < model[0].InfoTarjet; i++)
                        {
                            var exi = _context.UserCardNumberGroupData.Where(s => s.UserId == model[i].InfoTarjet.UserId && s.CardNumber == model[i].InfoTarjet.CardNumber).FirstOrDefault();
                            if (exi != null)
                            {
                                return false;
                            }
                            ...
                            {
                                var user = _context.Users.Where(x => x.UserId == model[i].InfoTarjet.UserId).FirstOrDefault();
                                if (user != null)
                                {
                                    if (model[i].InfoTarjet.ExpiritDate != null)
                                    {
                                        user.ExpiryDate = Convert.ToDateTime(model[i].InfoTarjet.ExpiritDate);
                                    }
                                    else
                                    {
                                        user.ExpiryDate = DateTime.Now;
                                    }
                                    if (model[i].InfoTarjet.StartDate != null)
                                    {
                                        user.StartDate = Convert.ToDateTime(model[i].InfoTarjet.StartDate);
                                    }
                                    else
                                    {
